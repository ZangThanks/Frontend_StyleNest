...
const TransactionsTableOrder = ({ orders }) => {
  ...
  const formatDate = (dateString) => {
    ...
    if (parts.length < 2) {
      console.error("Invalid date format (missing parts):", dateString);
      return dateString;
    }

    const time = parts[0]; // Giữ lại time vì cần dùng sau
    const date = parts[1]; // Lấy phần date
            ...
            {paginatedOrders.map((orders) => (
              <tr key={orders.id} className="border-b border-gray-200 h-16">
                ...
                <td className="text-gray-500">
                  {formatDate(orders.timeline[0].orderDate)}
                </td>
          ...
          <div className="relative bg-white border border-gray-200 p-6 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-fadeIn scale-100 transition-transform duration-300">
            ...
            <div className="flex items-center gap-4 mb-6">
              <img
                ...
                alt={selectedOrders.customerName}
            ...
            <div className="space-y-2 text-sm md:text-base text-gray-700 leading-relaxed border border-gray-300 rounded-lg p-4 bg-white shadow-sm">
              <p>
                <strong>Địa chỉ:</strong> {selectedOrders.address}
              </p>
              <p>
                <strong>Ngày đặt hàng:</strong>{" "}
                {formatDate(selectedOrders.timeline[0].orderDate)}
              </p>
              <p>
                <strong>Trạng thái:</strong> {selectedOrders.timeline[0].status}
              </p>
              <p>
                <strong>Tổng tiền:</strong>{" "}
                {selectedOrders.total.toLocaleString()}₫
              </p>
            </div>
                ...
                {selectedOrders.orderDetails.map((item, index) => (
                  ...
                  >
                    <img
                      ...
                      className="w-34 h-34 border border-gray-300 rounded object-cover shadow"
                    />
