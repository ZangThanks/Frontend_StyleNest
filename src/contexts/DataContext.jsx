...
export const DataProvider = ({ children }) => {
  ...
  const saveAccount = async (newAccount) => {
    try {
      ...
      return {
        success: false,
        error: error.response?.data?.message || "Đã xảy ra lỗi khi đăng ký",
      };
    }
  };
  ...
  const updateAccount = async (updatedAccount) => {
    try {
      const response = await axios.put(
        `${API}/api/accounts/${updatedAccount.id}`,
        updatedAccount
      );
  ...
  const updateUser = async (updatedUser) => {
    try {
      const response = await axios.put(
        `${API}/api/users/${updatedUser.id}`,
        updatedUser
      );
      setAccounts((prevAccounts) =>
        prevAccounts.map((user) =>
          user.id === updatedUser.id ? response.data.updatedUser : user
        )
      );
    } catch (error) {
      console.error("Lỗi khi cập nhật tài khoản:", error);
  ...
  const saveCustomer = async (customerData, imageFiles) => {
    try {
      ...
      const response = await axios.post(`${API}/api/customers`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
